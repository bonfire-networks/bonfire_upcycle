...
defmodule ValueFlows.EconomicEvent.EconomicEvents do
  ...
  * Various parts of the codebase that need to query for this (inc. tests)
  """
  def many(filters \\ []),
    do: {:ok, repo().many(Queries.query(EconomicEvent, filters))}
  ...
  * GraphQL resolver bulk resolution
  ...
  def preload_action(%{action_id: action_id} = event)
      when is_binary(action_id) do
    Map.put(event, :action, ValueFlows.Knowledge.Action.Actions.action!(action_id))
  end
  ...
  def preload_action(%{action: %{id: action_id}} = event)
      when is_binary(action_id) do
    Map.put(event, :action, ValueFlows.Knowledge.Action.Actions.action!(action_id))
  end

  def preload_action(%{action: %{label: label}} = event)
      when is_binary(label) do
    Map.put(event, :action, ValueFlows.Knowledge.Action.Actions.action!(label))
  end

  # fallback
  def preload_action(%{action: action} = event) do
    Map.put(event, :action, action)
  end
  ...
  def create(creator, event_attrs, extra_attrs \\ %{}) do
    create_somethings(
      creator,
      prepare_create_attrs(event_attrs, creator),
      extra_attrs
    )
  end
  ...
  defp create_somethings(
       ...
       when not is_nil(from_existing_resource) and
              not is_nil(to_existing_resource) and
              not is_nil(new_inventoried_resource) and
              new_inventoried_resource != %{} and
              from_existing_resource != to_existing_resource and
              from_existing_resource != new_inventoried_resource and
              to_existing_resource != new_inventoried_resource do
    error = "Oops, you cannot act on three resources in one event."
    warn("Events.create/3: " <> error)

    IO.inspect(
      event_with_three_resources: [
        %{
          resource_inventoried_as: from_existing_resource,
          to_resource_inventoried_as: to_existing_resource
        },
  ...
  defp create_somethings(
    ...
    new_resource_attrs =
      Map.put_new(
        ...
        :primary_accountable,
        Map.get(event_attrs, :receiver, creator)
      )
  ...
  defp create_somethings(
    ...
    new_resource_attrs =
      Map.put_new(
        new_inventoried_resource,
        :primary_accountable,
        Map.get(event_attrs, :provider, creator)
      )
  ...
  defp create_somethings(creator, %{action_id: action} = event_attrs, _)
       when is_binary(action) do
    create_with_action(
      creator,
      event_attrs,
      ValueFlows.Knowledge.Action.Actions.action!(action)
    )
  end
  ...
  defp create_with_action(
    ...
    create_somethings(creator, event_attrs, %{
      new_inventoried_resource:
        ...
        |> Map.merge(%{
          name: Map.get(event_attrs, :resource_name, "[resource name]")
        })
  ...
  defp create_with_action(
    ...
    log =
      "Events.create_with_action: decrementing+incrementing (eg. transfering or moving [part of] a resource)"
    ...
    new_inventoried_resource_attrs =
      with {:ok, fetched} <-
             EconomicResources.one([
               :preload_primary_accountable,
               id: resource_inventoried_as_id
             ]) do
        debug(
          log <>
        ...
        _ ->
          debug(
            log <>
              " creating a blank target resource (based on info from the event)"
          )
  ...
  defp create_with_action(
    ...
    log =
      "Events.create_with_action: decrementing (eg. consuming or lowering [part of] a resource)"
  ...
  defp create_resource_and_event(
         ...
         event_attrs,
         new_inventoried_resource,
         field_name \\ :resource_inventoried_as_id
       ) do
    new_resource_attrs = Map.put_new(new_inventoried_resource, :is_public, true)
  ...
  defp create_event(%{} = creator, new_event_attrs) do
    repo().transact_with(fn ->
      with :ok <- validate_user_involvement(creator, new_event_attrs),
           ...
           {:ok, event} <-
             repo().insert(EconomicEvent.create_changeset(creator, new_event_attrs)),
  ...
  defp post_create_event(event, creator, attrs) do
    with event = preload_all(event),
         ...
         {:ok, activity} <-
           ValueFlows.Util.publish(creator, event.action_id, event) do
  ...
  def update(user, %EconomicEvent{} = event, attrs) do
    repo().transact_with(fn ->
      ...
      with :ok <- validate_user_involvement(user, event),
           {:ok, event} <-
             repo().update(EconomicEvent.update_changeset(event, attrs)),
           {:ok, event} <- maybe_transfer_resource(event),
  ...
  defp maybe_transfer_resource(
       ...
       when action_id in ["transfer", "transfer-all-rights"] and
              not is_nil(to_resource_id) and
    ...
    with {:ok, to_resource} <-
         ...
         {:ok, to_resource} <-
           EconomicResources.update(to_resource, %{
             primary_accountable: receiver_id
           }) do
      ...
      _ ->
        debug(
          "Events.maybe_transfer_resource: did not transfer the resource (could not find or update the to_resource, or user not authorized to do so)"
        )
  ...
  defp validate_user_involvement(
       ...
       when (is_binary(provider) and is_binary(receiver) and
               provider == creator_id) or
  ...
  defp validate_user_involvement(creator, event) do
    error(
      "VF - Permission error, creator is #{inspect(creator)} and provider is #{inspect(event.provider)} and receiver is #{inspect(event.receiver)}"
    )
  ...
  defp validate_provider_is_primary_accountable(
    ...
    with {:ok, resource} <-
           EconomicResources.one([:preload_primary_accountable, id: resource_id]) do
  ...
  defp validate_provider_is_primary_accountable(
    ...
    if is_nil(resource.primary_accountable_id) or
      ...
      {:error,
       error(
         403,
         "You cannot do this because you are not accountable for that resource. Please contact #{e(resource, :primary_accountable, :character, :username, resource.primary_accountable_id)}"
       )}
    end
  end
  ...
  defp validate_receiver_is_primary_accountable(
         %{
           to_resource_inventoried_as_id: resource_id,
           receiver_id: receiver_id,
           provider_id: provider_id
         } = _event
       )
    ...
    with {:ok, resource} <-
           EconomicResources.one([:preload_primary_accountable, id: resource_id]) do
      if is_nil(resource.primary_accountable_id) or
           receiver_id == resource.primary_accountable_id or
           provider_id == resource.primary_accountable_id do
        :ok
      else
        {:error,
         fail(
           403,
           "You cannot do this because you are not accountable for the target resource. Please contact #{e(resource, :primary_accountable, :character, :username, resource.primary_accountable_id)})"
         )}
      end
    end
  end
  ...
  def prepare_attrs(attrs, creator \\ nil) do
    ...
    |> maybe_put(
      :to_resource_inventoried_as_id,
      attr_get_id(attrs, :to_resource_inventoried_as)
    )
